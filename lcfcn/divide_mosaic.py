...
def generate_dots(h_start, h_end, w_start, w_end, clicks, mosaic_image):
    ...
    for click in clicks:
        if click[1] <= h_end and click[1] >= h_start and click[0] <= w_end and click[0] >= w_start:
            h_int = int(click[1] / (JUMP))
            w_int = int(click[0] / (JUMP))
            ...
            dots = cv2.circle(dots, (x, y), radius=0, color=(0, 0, 255), thickness=1)
    return dots
...
for file in images:
    ...
    for h_i, h in enumerate(range(0, height, JUMP)):
        for w_i, w in enumerate(range(0, width, JUMP)):
            ...
            '''
            if w_i == 5:
                image = cv2.rectangle(image, (h, w), (h+JUMP, w+JUMP), 2, 1)
                cv2.imwrite(f'{MOSAIC_DIR}file_{h}_{w}.png', image)
                break
            '''